...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect, QueenAnt) and insect.removable == False:
                return None
...
class Ant(Insect):
    ...
    def can_contain(self, ant):
        # if self.container and not ant.container:
        #     if self.ant == None:
        #         return True
        # return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        # BEGIN Problem 3B
        def nearest_place(place, count):
            if hive == place or count > self.max_range:
                return None
            elif place.bees and place != hive and \
            count >= self.min_range and count < self.max_range:
                return place
            else:
                return nearest_place(place.entrance, count+1)
        bee_place = nearest_place(self.place, 0)
        if bee_place == None:
            return None
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 6A
        beees = [b for b in self.place.bees]
        for b in beees:
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.place.bees != []:
            if self.digesting == 0:
                bee = self.place.bees[int(random.random() * len(self.place.bees))]
                self.eat_bee(bee)
            else:
                self.digesting -= 1
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        ...
        if self.ant != None:
            self.ant.action(colony)
        bees = [b for b in self.place.bees]
        for b in bees:
            b.reduce_armor(self.damage)
        # END Problem 8
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    food_cost = 7
    implemented = True
    first = True   # Change to True to view in the GUI
    ants_behind = []
    # END Problem 9
    ...
    def action(self, colony):
        ...
        if not self.first:
            ...
            ########Return ants that are behind
            placex = self.place # make a copy of the current place
            while placex.exit:
                placex = placex.exit
                if placex.ant:
                    o_ant = placex.ant # out-side ant
                    if o_ant.container: # o_ant is a bodyguard
                        if o_ant.ant and o_ant.ant not in self.ants_behind: # contains an ant
                            self.ants_behind.append(o_ant.ant)
                            if hasattr(o_ant.ant, 'damage'): # double if it has damage
                                o_ant.ant.damage *= 2
                        if o_ant not in self.ants_behind:
                            self.ants_behind.append(o_ant)
                            if hasattr(o_ant, 'damage'):
                                o_ant.damage *= 2
                    elif o_ant not in self.ants_behind:    # just an ordinary ant
                            self.ants_behind.append(o_ant)
                            if hasattr(o_ant, 'damage'):
                                o_ant.damage *= 2
...
def make_slow(action):
    ...
    """
    # BEGIN Problem EC
    def new_make_slow(colony):
        if colony.time % 2 == 0:
            return action(colony)
        return None
    return new_make_slow
    # END Problem EC
...
def make_stun(action):
    ...
    """
    # BEGIN Problem EC
    def new_make_stun(colony):
        return None
    return new_make_stun
    # END Problem EC
